...
const taskResource = (taskId: string, suffix = "") =>
	`/tasks/${encodeURIComponent(taskId)}${suffix}`;
const modelingResource = (taskId: string, suffix: string) =>
	`/modeling/${encodeURIComponent(taskId)}${suffix}`;
...
export function getTaskWorkspace(task_id: string) {
	return request.get<TaskWorkspaceSnapshot>(
		taskResource(task_id, "/workspace"),
	);
}

/** 保存任务页中的用户补充消息。 */
export function sendTaskMessage(task_id: string, content: string) {
	return request.post<Message>(taskResource(task_id, "/messages"), { content });
}
...
export function getServiceStatus() {
	return request.get<ServiceHealth>("/status");
}

export function cancelTask(task_id: string) {
	return request.post<{ success: boolean; message: string }>(
		modelingResource(task_id, "/cancel"),
	);
}
...
export function getResumeOptions(task_id: string) {
	return request.get<ResumeOptions>(
		modelingResource(task_id, "/resume-options"),
	);
}
...
export function resumeTask(task_id: string, node_id: string) {
	return request.post<{
		...
		node_id: string;
		message: string;
	}>(modelingResource(task_id, "/resume"), { node_id });
}

/** 获取刷新后仍有效的待人工审核节点。 */
export function getPendingApproval(task_id: string) {
	return request.get<ApprovalStatus>(modelingResource(task_id, "/approval"));
}
...
export function submitApproval(
	...
	return request.post<{
		...
		node_id: string;
		message: string;
	}>(modelingResource(task_id, "/approval"), payload);
}
