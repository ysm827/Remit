...
export type ToolName = "execute_code" | "search_scholar";

export type ToolMessage = EventOf<
	"tool",
	{
		tool_name: ToolName;
...
export type SystemMessage = EventOf<"system", { type: SystemMessageType }>;
export type UserMessage = EventOf<"user">;

type AgentEvent<Role extends AgentType> = EventOf<
	"agent",
	{ agent_type: Role }
>;

export type AgentMessage = AgentEvent<AgentType>;
export type ModelerMessage = AgentEvent<AgentType.MODELER>;
export type CoordinatorMessage = AgentEvent<AgentType.COORDINATOR>;
export type CoderMessage = AgentEvent<AgentType.CODER>;
export type WriterMessage = AgentEvent<AgentType.WRITER> & {
	sub_title?: string;
};
...
export type ExecutionFormat = (typeof executionFormats)[number];

type ExecutionRecord<Kind extends string, Extra extends object = object> = {
	res_type: Kind;
	msg?: string;
} & Extra;

export type StdOutExecution = ExecutionRecord<"stdout">;
export type StdErrExecution = ExecutionRecord<"stderr">;
export type ResultExecution = ExecutionRecord<
	"result",
	{ format: ExecutionFormat }
>;
export type ErrorExecution = ExecutionRecord<
	"error",
...
export type ScholarMessage = ToolMessage & {
	tool_name: "search_scholar";
	input: Record<string, never>;
	output: string[];
};

export type InterpreterMessage = ToolMessage & {
	msg_type: "tool";
